...
public class FullBinaryTreePostPre
{
	...
	// current subarray in post[]
	 node constructTreeUtil(int pre[], int post[], int l,
								int h, int size)
	{
	...
	{
		FullBinaryTreePostPre fullBinaryTreePostPre = new FullBinaryTreePostPre();
		int pre[] = { 1, 2, 4, 8, 9, 5, 3, 6, 7 };
		int post[] = { 8, 9, 4, 5, 2, 6, 7, 3, 1 };

		int size = pre.length;
		node root = fullBinaryTreePostPre.constructTree(pre, post, size);

		System.out.println("Inorder traversal of the constructed tree:");
		fullBinaryTreePostPre.printInorder(root);
	}
}
